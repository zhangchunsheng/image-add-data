 ...
 * @email zhangchunsheng@luomor.com
 * @version V1.0
 * @date 2016-03-30
 * @copyright Copyright (c) 2010-2018 Luomor Inc. (http://www.luomor.com)
 */
...
class Bootstrap extends \Yaf\Bootstrap_Abstract {
    public function _initErrorHandler(Yaf\Dispatcher $dispatcher) {
        $dispatcher->setErrorHandler(array(get_class($this), 'error_handler'), E_ALL);
    }
    ...
    public function _initLoader(Yaf\Dispatcher $dispatcher) {
        ...
        $loader->registerLocalNameSpace($namespaces);
    }
    ...
    public function _initDatabase(Yaf\Dispatcher $dispatcher) {
        $config = Yaf\Registry::get("config")->databases->toArray();
        \LM\Db\Manager::setConfigEx($config);
    }

    public function _initPlugins(Yaf\Dispatcher $dispatcher) {
        $dispatcher->registerPlugin(new LogPlugin());

        $this->config->application->protect_admin &&
