...
class Timer {
    ...
    public static function startNewTimer($microtime = null) {
     ...
     * @return float|int
     */
    public function getMicrosecond($last = false) {
     ...
     * @return float
     */
    public function getMs($last = false) {
        return $this->getMillisecond($last);
    }

    public function getMillisecond($last = false) {
        if (!$last || $this->end_time == 0) {
            $this->stop();
        }
     ...
     * @return float
     */
    public function getSecond($last = false) {
        if (!$last || $this->end_time == 0) {
            $this->stop();
        }
     ...
     * @return float
     */
    public function getMinute($last = false) {
        if (!$last || $this->end_time == 0) {
            $this->stop();
        }
