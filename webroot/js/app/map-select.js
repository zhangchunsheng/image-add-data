                    ...
                    if(selecter.settings.readonly == 1) {
                        ...
                        if(checkHtml == "") {
                            ...
                            $(".map-city").bsSuggest('init', {
                                clearable: true,
                                url: "/region/getListForErp?only_list=1&has_short=1",
                                idField: "short",
                                keyField: "cn",
                                effectiveFields: ["short", "cn", "cn_phonetic"],
                                searchFields: ["short", "cn", "cn_phonetic"],
                                effectiveFieldsAlias: {short: "城市简码", cn: "城市名称", cn_phonetic: "汉语拼音"},
                                ignorecase: true,
                                fnProcessData: function(result) {
                                    var value = [];
                                    for(var i in result.result) {
                                        value.push(result.result[i]);
                                    }
                                    return {value: value};
                                }
